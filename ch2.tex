...
\section{Нет ничего более непостоянного, чем переменная}\index{variable}
...
Мы также можем сказать Python, чтобы переменная \code{fred} указывала на что-нибудь другое:
...
В примере выше мы создаем переменную (или ярлык) \code{john}, который указывает на то же самое значение, на которое указывает переменная \code{fred}.
...
Мы можем так сделать, потому что имена переменных могут состоять из букв, цифр и знаков подчёркивания (\_)---однако, не могут начинаться с цифры.  Даже вернувшись через 10 лет, смысл переменной `number\_of\_students' будет тебе понятен.  Ты можешь напечатать:
...
%\end{verbatim}\footnote{ffff ffff}
\end{listing}

\section{Использование Переменных}\index{Variables}
